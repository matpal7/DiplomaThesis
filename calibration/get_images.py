...
# def getCalibrationPhoto_old_delete(camara_index_left, camera_index_right, save_dir, frame_size=(width, height), chessboard_size=(7,6)):
...
#
#     cap_l_4K = opencv_open_camera(camara_index_left, (width_4K, height_4K))
#     cap_r_4K = opencv_open_camera(camera_index_right, (width_4K, height_4K))
#
...
def getCalibrationPhoto(camera_index_left, camera_index_right, save_dir,
                        frame_size_hd=(1280, 720), frame_size_4k=(3840, 2160),
                        chessboard_size=(7, 6)):

    # Open cameras for HD
    cap_l = opencv_open_camera(camera_index_left, frame_size_4k)
    cap_r = opencv_open_camera(camera_index_right, frame_size_4k)


    calib_dir = os.path.join(save_dir, "calibration")
    os.makedirs(calib_dir, exist_ok=True)
    num = findMaxNumber(calib_dir)   # <-- assumes you already have this function
    print("Started", num)

    frame_size_preview = (680, 480)
    ...
    while cap_r.isOpened():
        ...
        resized_frame_L = cv2.resize(imgL, frame_size_preview)
        resized_frame_R = cv2.resize(imgR, frame_size_preview)
        img_concat_h = cv2.hconcat([resized_frame_L, resized_frame_R])
        cv2.imshow('Calibration Preview', img_concat_h)

        k = cv2.waitKey(0)
        if k == 27:   # ESC = exit
            break
        elif k == ord('s'):  # Save both HD & 4K
            # Save HD
            cv2.imwrite(os.path.join(calib_dir, f"{num}_left.png"), image_L)
            cv2.imwrite(os.path.join(calib_dir, f"{num}_right.png"), image_R)
...
def save_cameras_on_click(camara_index_left, camera_index_right, calib_dict, frame_size_RealSense=(1280, 720), save_dir="dataset_default"):
    """
...
if __name__ == '__main__':
    ...
    # getCalibrationPhoto(3, 1, save_dir, chessboard_size=chessboard_size)
