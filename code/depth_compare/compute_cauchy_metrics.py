...
def compute_cauchy_metrics(depth_gt: np.ndarray, pred_warped: np.ndarray, valid_mask: np.ndarray, gamma: float) -> dict[str, float]:
    ...
    out = {
        ...
        "n_pixels_outside_ci": int((~within_ci).sum()),
        "outside_ci_pct": float(np.mean(~within_ci) * 100.0),
...
def main() -> None:
    ...
    gt_data_dir, relative_pose_path, calib_rgbd_path, calib_stereo_path, depth_estimation_dir, depth_comparison_dir = (
    ...
    for nn_key, nn_value in get_depth_estimation_network().items():
        ...
        for gt_frame, pred_depth in tqdm(zip(gt_frames, pred_depth_maps), desc=f"[{nn_value}]", total=len(pred_depth_maps)):
            ...
            valid_mask = np.isfinite(pred_warped) & (pred_warped > 0) & np.isfinite(depth_gt) & (depth_gt > 0)
        ...
        summary = {"nn_name": nn_key, "zed_gamma": zed_gamma, "confidence": CONFIDENCE}
        for col in per_image_df.columns:
            if col in {"image_id", "nn_name"}:
                continue
            summary[col] = float(per_image_df[col].sum() if col in {"n_pixels", "n_pixels_outside_ci"} else per_image_df[col].mean())
    ...
    if summaries:
        ...
        available_cols = [col for col in table_cols if col in df_summary.columns]

        print(
            df_summary[available_cols].to_string(
                index=False,
                formatters={
                    "median_abs_z": "{:.4f}".format,
                    "mean_abs_z": "{:.4f}".format,
                    "within_ci_pct": "{:.2f}".format,
                    "outside_ci_pct": "{:.2f}".format,
        ...
        best_lowest_median_abs_z = df_summary.loc[df_summary["median_abs_z"].idxmin(), "nn_name"]
        best_lowest_mean_abs_z = df_summary.loc[df_summary["mean_abs_z"].idxmin(), "nn_name"]
        best_highest_within_ci = df_summary.loc[df_summary["within_ci_pct"].idxmax(), "nn_name"]
        best_highest_within_1gamma = df_summary.loc[df_summary["within_1gamma_pct"].idxmax(), "nn_name"]
        best_highest_within_2gamma = df_summary.loc[df_summary["within_2gamma_pct"].idxmax(), "nn_name"]
        best_highest_mean_log_lik = df_summary.loc[df_summary["mean_log_lik"].idxmax(), "nn_name"]
        best_lowest_ks_stat = df_summary.loc[df_summary["cauchy_ks_stat"].idxmin(), "nn_name"]

        print(f"  • lowest median_abs_z        : {best_lowest_median_abs_z}")
        print(f"  • lowest mean_abs_z          : {best_lowest_mean_abs_z}")
        print(f"  • highest within_ci_pct      : {best_highest_within_ci}")
        print(f"  • highest within_1gamma_pct  : {best_highest_within_1gamma}")
        print(f"  • highest within_2gamma_pct  : {best_highest_within_2gamma}")
        print(f"  • highest mean_log_lik       : {best_highest_mean_log_lik}")
        print(f"  • lowest cauchy_ks_stat      : {best_lowest_ks_stat}")
