...
def _infer_calibration_size(path: Path, suffix: str) -> tuple[int, int] | None:
    """
    Return calibration image size as (width, height), if available.
    """
    path = Path(path)
    if path.suffix.lower() in {".yaml", ".yml"}:
        ...
        try:
            ...
            size_node = fs.getNode("image_size").mat()
            if size_node is not None:
                vals = np.asarray(size_node).reshape(-1)
                if vals.size >= 2:
    ...
    if path.suffix.lower() == ".npy":
        ...
        key = "img_dim_l" if suffix == "left" else "img_dim_r"
        if key not in calib:
            return None
        vals = np.asarray(calib[key]).reshape(-1)
        if vals.size < 2:
            return None
        return int(vals[0]), int(vals[1])
...
def _parse_size_arg(size_arg: str | None) -> tuple[int, int] | None:
    if size_arg is None:
        return None
    parts = size_arg.lower().replace("x", " ").split()
    if len(parts) != 2:
        raise ValueError(f"Invalid size format '{size_arg}'. Use WIDTHxHEIGHT, e.g. 1280x720.")
...
def _compute_metrics(pred_target_m: np.ndarray, gt_target: np.ndarray, gt_depth_scale: float, valid_projected: np.ndarray) -> dict:
    ...
    total_pixels = int(pred_target_m.size)
    projected_pixels = int(np.count_nonzero(valid_projected))
    ...
    if n == 0:
        return {
            "num_valid_pixels": 0,
            "num_projected_pixels": projected_pixels,
            "projected_coverage": float(projected_pixels / max(total_pixels, 1)),
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Calibration size WIDTHxHEIGHT for source K (overrides auto-detect).",
    )
    parser.add_argument(
        "--target-calib-size",
        type=str,
        default=None,
        help="Calibration size WIDTHxHEIGHT for target K (overrides auto-detect).",
    )
...
def main() -> None:
    ...
    src_calib_size = _parse_size_arg(args.source_calib_size) or _infer_calibration_size(args.source_calib, args.source_suffix)
    tgt_calib_size = _parse_size_arg(args.target_calib_size) or _infer_calibration_size(args.target_calib, args.target_suffix)

    src_depth_size = (source_depth.shape[1], source_depth.shape[0])
    tgt_depth_size = (target_depth.shape[1], target_depth.shape[0])

    if src_calib_size is not None and src_calib_size != src_depth_size:
        print(f"[INFO] Scaling source intrinsics from calib size {src_calib_size} to source depth size {src_depth_size}")
        k_source = scale_intrinsics(k_source, src_calib_size, src_depth_size)
    elif src_calib_size is None:
        print("[WARN] Source calibration image size unknown; intrinsics are not scaled.")

    if tgt_calib_size is not None and tgt_calib_size != tgt_depth_size:
        print(f"[INFO] Scaling target intrinsics from calib size {tgt_calib_size} to target depth size {tgt_depth_size}")
        k_target = scale_intrinsics(k_target, tgt_calib_size, tgt_depth_size)
    elif tgt_calib_size is None:
        print("[WARN] Target calibration image size unknown; intrinsics are not scaled.")
